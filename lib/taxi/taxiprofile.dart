...
class ProfileTaxi extends StatefulWidget {
  const ProfileTaxi({Key? key}) : super(key: key);

  @override
  State<ProfileTaxi> createState() => _ProfileTaxiState();
}

class _ProfileTaxiState extends State<ProfileTaxi>
  ...
  _update() async {
    setState(() {
     ...
     print(data);
     var res = await Api().authData(data, '/taxih/updatetaxi/' + _id);
     var body = json.decode(res.body);
  ...
  _updatepass() async {
    ...
    print(data);
    var res = await Api().authData(data, '/taxih/updatetaxipass/' + login_id);
    var body = json.decode(res.body);
